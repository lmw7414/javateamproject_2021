...
public class Game {
	...
	private final String[] Word = { "apple", "kiwi", "banana", "soup", "strawberry", "pizza", "burger", "chicken",
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				...
				if (gamecard1[value] == 2) {
					...
					gameboard1[i][j] = new Card(Word[value - 1], Status.back);
					gamecard1[value]++;
				}
			}
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				if (gameboard[i][j].getCardState() == Status.back)
					...
					System.out.printf("[%-10s]", gameboard[i][j].getCardWord());
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 6; j++) {
				...
				if (gamecard2[value] == 2) {
					...
					gameboard2[i][j] = new Card(Word[value - 1], Status.back);
					gamecard2[value]++;
				}
			}
		}
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 8; j++) {
				...
				if (gamecard3[value] == 2) {
					...
					gameboard3[i][j] = new Card(Word[value - 1], Status.back);
					gamecard3[value]++;
